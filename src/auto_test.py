...
def run_backtest(strat_name_run_backtest, strat_id_run_backtest, strat_version_run_backtest, pair_run_backtest, recommanded_run_backtest, backtest_date_run_backtest, exchange_select_run_backtest):
    ...
    tools.log(f"[RUN][run_backtest][INFO] : Selected exchange = {exchange}")
    tools.log(f"[RUN][run_backtest][INFO] : Testing period = {backtest_date_period}, from {backtest_date_start} to {backtest_date_end}")
    if not api.backtest_has_failed(
        ...
        tools.log("[RUN][run_backtest] : Test in progress, Please Wait...")
        # set date into input
        ...
        if not windows_handle:
            tools.log("[RUN][run_backtest][ERROR] : depth analysis button is break on kryll side, period canceled")
        ...
        try:
            ...
            if not depth_analysis_page_loaded:
                tools.log("[RUN][run_backtest][WARNING] : depth analysis tab seems to don't load, refresh and retry...")
                # retry
                sel_tools.refresh()
                depth_analysis_page_loaded = sel_tools.wait_for_element(css.ADVANCED_ANALYSE_TRADE, 120)
            ...
            tools.log("[RUN][run_backtest][ERROR] : Sending backtest : Exception occured : " + str(error))
        sel_tools.driver.close()
        sel_tools.driver.switch_to.window(sel_tools.driver.window_handles[0])
        if send_ok:
            tools.log("[RUN][run_backtest][INFO] : Done.")
            return True
        tools.log("[RUN][run_backtest][ERROR] : Error during sending the result, period canceled.")
        raise Exception("Error during backtest")
    return False
...
def run():
    ...
    for strat_id in strat_ids:
        ...
        for exchange in final_exchanges:
            ...
            if exchange:
                tools.log(f"[RUN][run][INFO] : Testing on exchange {exchange}")
            ...
            for i in total_pairs_list:
                ...
                if "pair" in user.config_file:
                    if recommended == 0 and not pair.replace(" / ", "/") in user.config_file["pair"]:
                        tools.log(f"[RUN][run][INFO] : Pair {pair} skipped", True)
                        continue
                ...
                if recommended == 1:
                    if "skip_recommended_pair" in user.config_file and user.config_file["skip_recommended_pair"] == "y" and force_pair == 0:
                        continue
                tools.log(f"[RUN][run][INFO] : pair = {pair}, recommended = {recommended}")
                ...
                try:
                    ...
                    tools.log(fr"[RUN][run][WARNING] : /!\ Error recommanded pair {pair} not listed on {exchange}")
                    continue
                ...
                tools.log("[RUN][run][INFO] : run backtest for pair " + pair + " for all selected periods")
                for backtest_date in backtest_dates:
                    try:
                        ...
                        tools.log("[RUN][run][ERROR] : Backtest Failed.")
                        tools.log("[RUN][run][ERROR] : You can see the screenshot on this file : " + screenshot_name)
            tools.log(f"[RUN][run][INFO] : strat backtested : {strat_name}, version : {strat_version} : Done")
...
while COUNT_QUICK_FAIL < 3:
    ...
    try:
        ...
        sel_tools.save_screenshot(SCREENSHOT_NAME)
        tools.log("[RUN][MAIN][ERROR] : Exception occured on execution " + str(EXECUTION_ID) + " : " + str(error))
        tools.log("[RUN][MAIN][ERROR] : You can see the screenshot on this file : " + SCREENSHOT_NAME)
        tools.log("[RUN][MAIN][ERROR] : Retry...")
    end = time.time()
    elapsed = end - start
    if elapsed < 30:
        COUNT_QUICK_FAIL = COUNT_QUICK_FAIL + 1
        tools.log("[RUN][MAIN][ERROR] : Quick fail : " + str(COUNT_QUICK_FAIL) + "/3")
    else:
        COUNT_QUICK_FAIL = 0
